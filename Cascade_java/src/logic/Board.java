...
// ***** VDMTOOLS START Name=Board KEEP=NO
  public Board () throws CGException {
    try {
      vdm_init_Board();
      //new MATH().srand(Math.abs((int)System.currentTimeMillis()));
      {
...
// ***** VDMTOOLS START Name=generateColor KEEP=NO
  public Object generateColor () throws CGException {
    ...
    try {
      Number num = null;
      MATH obj_2 = new MATH();
      num = (int)(Math.random() * 3); //not calling rand
      if (num.intValue() == 0) 
    ...
    try {
      ...
      if (n_pieces.intValue() >= 15) {
        ...
        if (pieces.containsKey(pos)) 
          level_43 = (Cell)pieces.get(pos);
        else 
          level_43 = new Cell();
        level_43.color = new quotes.bomb();
        pieces.put(pos, level_43);
        if (!this.inv_Board().booleanValue()) 
          UTIL.RunTime("Instance invariant failure in Board");
      }
      fixBoard();
