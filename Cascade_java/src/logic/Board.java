...
// ***** VDMTOOLS START Name=print KEEP=NO
  public Boolean print () throws CGException {
    ...
    try {
      Number count = new Integer(0);
      {
        Set res_s_4 = new HashSet();
        {
          Set e1_set_8 = new HashSet(pieces.keySet());
          Position x = null;
          Set tmpSet_21 = new HashSet(e1_set_8);
          for (Iterator enm_20 = tmpSet_21.iterator(); enm_20.hasNext(); ) {
            Position elem_19 = (Position)enm_20.next();
            /* x */
            x = elem_19;
            Boolean pred_9 = null;
            if ((pred_9 = Boolean.valueOf((x.posX).intValue() >= 1)).booleanValue()) 
              pred_9 = Boolean.valueOf((x.posX).intValue() < LimiteX.intValue());
            if (pred_9.booleanValue()) {
              res_s_4.add(x);
            }
          }
        ...
        Set iset_22 = res_s_4;
        Set tmpSet_55 = new HashSet(iset_22);
        for (Iterator enm_54 = tmpSet_55.iterator(); enm_54.hasNext(); ) {
          Position elem_1 = (Position)enm_54.next();
          Position p = null;
          /* p */
          p = elem_1;
          Boolean cond_23 = null;
          if ((cond_23 = Boolean.valueOf(UTIL.equals((((Cell)pieces.get(p)).color), new quotes.empty()))).booleanValue()) 
            cond_23 = Boolean.valueOf((p.posY).intValue() == 0);
...
// ***** VDMTOOLS START Name=distLeft#1|Number KEEP=NO
  public Number distLeft (final Number posx) throws CGException {
    ...
    try {
      if (!this.pre_distLeft(posx).booleanValue()) 
        UTIL.RunTime("Precondition failure in distLeft");
      {
        List dm = null;
        List tmpSeq_2 = new ArrayList();
        for (int count_3 = ((-posx.intValue()) + 1); count_3 <= 0; count_3++) {
          Number x = new Integer(count_3);
          tmpSeq_2.add(x);
        }
        dm = tmpSeq_2;
        if (UTIL.equals((((Cell)pieces.get(new Position(posx, new Integer(0)))).color), new quotes.empty())) 
          while (dm.size() > 0) {
            Number h = UTIL.NumberToInt(dm.get(0));
            dm = new ArrayList(dm.subList(1, dm.size()));
            if (!this.inv_Board().booleanValue()) 
              UTIL.RunTime("Instance invariant failure in Board");
            if (!UTIL.equals((((Cell)pieces.get(new Position(new Integer(-h.intValue()), new Integer(0)))).color), new quotes.empty())) 
...
// ***** VDMTOOLS START Name=pre_distLeft#1|Number KEEP=NO
  public Boolean pre_distLeft (final Number posx) throws CGException {
    return Boolean.valueOf(posx.intValue() >= 1);
  }
// ***** VDMTOOLS END Name=pre_distLeft#1|Number
...
// ***** VDMTOOLS START Name=distRight#1|Number KEEP=NO
  public Number distRight (final Number posx) throws CGException {
    ...
    try {
      if (!this.pre_distRight(posx).booleanValue()) 
        UTIL.RunTime("Precondition failure in distRight");
      {
        List dm = null;
        List tmpSeq_2 = new ArrayList();
        for (int count_3 = (posx.intValue() + 1); count_3 <= LimiteX.intValue(); count_3++) {
          Number x = new Integer(count_3);
          tmpSeq_2.add(x);
        }
        dm = tmpSeq_2;
        if (UTIL.equals((((Cell)pieces.get(new Position(posx, new Integer(0)))).color), new quotes.empty())) 
          while (dm.size() > 0) {
            ...
            if (!this.inv_Board().booleanValue()) 
              UTIL.RunTime("Instance invariant failure in Board");
            if (!UTIL.equals((((Cell)pieces.get(new Position(h, new Integer(0)))).color), new quotes.empty())) 
              return h;
          }
        return posx;
      }
...
// ***** VDMTOOLS START Name=pre_distRight#1|Number KEEP=NO
  public Boolean pre_distRight (final Number posx) throws CGException {
    Boolean varRes_2 = null;
    if ((varRes_2 = Boolean.valueOf(posx.intValue() >= 0)).booleanValue()) 
      varRes_2 = Boolean.valueOf(posx.intValue() < LimiteX.intValue());
    return varRes_2;
  }
// ***** VDMTOOLS END Name=pre_distRight#1|Number
